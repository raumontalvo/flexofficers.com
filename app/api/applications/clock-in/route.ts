...
import { validateClockInRequest } from "./rules";
...
function parseCoordinates(latitude: unknown, longitude: unknown) {
  ...
  return {
    clockInLatitude: parsedLatitude,
    clockInLongitude: parsedLongitude,
  };
}
  ...
  try {
    ...
    if (!actor || actor.role !== UserRole.OFFICER) {
      return NextResponse.json(
        { error: "Only officer accounts can clock in." },
        { status: 403 }
      );
    }

    const body = (await req.json()) as ClockInPayload;
    const applicationId =
    ...
    if (existing.officer.user.clerkId !== clerkUser.id) {
      return NextResponse.json(
        { error: "You can only clock in to your own assignments." },
        { status: 403 }
      );
    }

    const validation = validateClockInRequest(existing);
