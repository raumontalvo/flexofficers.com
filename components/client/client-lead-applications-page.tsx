...
import {
  CLIENT_APPLICATIONS_PAGE_SIZE,
  filterClientApplicationsByTab,
  formatClientApplicationsPagination,
  getClientApplicationDisplayStatusLabel,
  getClientApplicationTabCounts,
  paginateClientApplications,
  searchClientApplications,
  type ClientApplicationDisplayStatus,
  type ClientApplicationPageTab,
  type ClientApplicationsPageStats,
  type SerializedClientLeadApplication,
} from "@/lib/client-applications-page";
import { cn } from "@/lib/cn";

type ClientLeadApplicationsPageProps = {
  applications: SerializedClientLeadApplication[];
  stats: ClientApplicationsPageStats;
};

const TAB_ORDER: ClientApplicationPageTab[] = [
  "all",
  "pending",
  "hired",
  "declined",
...
const STATUS_BADGE_STYLES: Record<ClientApplicationDisplayStatus, string> = {
...
function ApplicationStatusBadge({ status }: { status: ClientApplicationDisplayStatus }) {
  return (
    <span
      className={cn(
    ...
    >
      {getClientApplicationDisplayStatusLabel(status)}
    </span>
  );
}
...
function EmptyState({ filtered }: { filtered: boolean }) {
  ...
  const copy = t.dashboard.clientLeadApplications;
...
function MobileApplicationCard({ application }: { application: SerializedClientLeadApplication }) {
  const { t } = useLandingLanguage();
  const copy = t.dashboard.clientLeadApplications.table;

  return (
    <Link
      href={application.profileHref}
...
export function ClientLeadApplicationsPage({
  applications,
  stats,
}: ClientLeadApplicationsPageProps) {
  const { t } = useLandingLanguage();
  const copy = t.dashboard.clientLeadApplications;
  const nav = t.appNav;
  const unreadCount = useUnreadNotificationCount();
  const [activeTab, setActiveTab] = useState<ClientApplicationPageTab>("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [page, setPage] = useState(1);

  const tabLabels: Record<ClientApplicationPageTab, string> = {
    all: copy.tabs.all,
    pending: copy.tabs.pending,
    hired: copy.tabs.hired,
    declined: copy.tabs.declined,
    withdrawn: copy.tabs.withdrawn,
  };

  const tabCounts = useMemo(() => getClientApplicationTabCounts(applications), [applications]);

  const filteredApplications = useMemo(() => {
    const byTab = filterClientApplicationsByTab(applications, activeTab);
    return searchClientApplications(byTab, searchQuery);
  }, [activeTab, applications, searchQuery]);

  const pagination = useMemo(
    () => paginateClientApplications(filteredApplications, page, CLIENT_APPLICATIONS_PAGE_SIZE),
    [filteredApplications, page]
  );

  const paginationLabel = formatClientApplicationsPagination(
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-white/[0.06] bg-white/[0.02] text-[11px] uppercase tracking-wide text-fo-text-muted">
              <tr>
                <th className="px-4 py-3 font-semibold">{copy.table.applicant}</th>
              ...
              {!hasAnyApplications || showFilteredEmpty ? (
                ...
                pagination.items.map((application) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <p className="font-medium text-fo-text">{application.leadTitle}</p>
                      <p className="mt-1 inline-flex items-center gap-1 text-xs text-fo-text-muted">
                        <LocationIcon className="h-3.5 w-3.5 text-blue-400" />
                        {application.leadLocation}
                      </p>
