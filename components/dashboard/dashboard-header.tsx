...
export function DashboardHeader({
  ...
  const displayName = firstName?.trim() || nav.officerFallback;
    ...
    <header className={cn("fo-dashboard-header px-3 py-3 sm:px-4 sm:py-3.5", className)}>
      <div className="flex items-center justify-between gap-4 md:hidden">
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium text-fo-text-muted">{t.common.welcomeBack}</p>
        ...
        <Link
          ...
          aria-label={nav.aria.viewProfile}
        >
          <ProfileAvatar
            name={avatarName}
            src={imageUrl}
      ...
      <div className="hidden items-center justify-between gap-3 md:flex">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-fo-text-muted">{copy.headerSubtitle}</p>
        </div>
